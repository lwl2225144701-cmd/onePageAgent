...
元素类型只有：image, text, sticker, decoration, date_tag, mood_tag, weather_tag。
背景素材使用 image + props.role=background；边框使用 decoration + props.role=frame；胶带使用 decoration + props.role=tape。
素材的 material_id 和 url 都必须写在 props 内，不能放在元素顶层。
...
- “本次最终建议使用素材 selected_materials”是从 reviewed candidates 中进一步压缩出的安全素材包；优先只使用这些素材，除非 fallback_mode=neutral_minimal 且素材为空
- 最终输出中的每个 image/sticker/decoration，其 props.material_id 和 props.url 必须逐字来自 selected_materials；禁止从 few-shot 的 `$background...` 等符号引用中复制占位值
- few-shot 中的 `$...` 仅表示“复用输入字段”的映射关系，不是真实素材、日期或天气；最终 JSON 禁止出现任何 `$...`、`<...>` 或示例素材引用
- few-shot 只用于学习构图、层级、密度和留白，不得复制示例的猫咪文案、日期、天气、心情或素材
- 如果 selected_materials 中有 background，可以使用 0-1 个；如果有 focal_sticker，最多使用 1 个；supporting_sticker 最多 2 个；decoration/tape/frame 合计最多 3 个
...
- 背景插画只负责氛围，使用 image + role=background，透明度不得超过 layout_policy.background.max_opacity；默认 fit=contain，不能盖住标题和正文可读性
...
- frame 只能作为页面边缘装饰；tape 只能作为窄条胶带点缀，不能横穿正文、日期、天气或人脸/主体
- 缺少 frame、decoration 或 tape 时直接省略，不得生成空元素或虚构素材；缺少 background 时使用 page.background 暖白底
...
输出格式参考如下，字段结构必须保持一致，但文案和素材只能使用本次输入：
```json
{{
  "page": {{
    "width": 1080,
  ...
  "elements": [
    {{
      "type": "image",
      "props": {{"material_id": "<selected_material_id>", "url": "<selected_material_url>", "role": "background", "x": 76, "y": 420, "w": 928, "h": 820, "fit": "contain", "opacity": 0.12}},
      "z_index": 0
    }},
    {{
      "type": "text",
      "props": {{"role": "body", "content": "<current_user_content>", "font": "handwriting", "size": 42, "color": "#5C4A3A", "x": 80, "y": 1180, "w": 920, "h": 420, "align": "left", "lineHeight": 1.8}},
      "z_index": 30
    }},
    {{
      "type": "sticker",
      "props": {{"material_id": "<selected_material_id>", "url": "<selected_material_url>", "role": "focal_sticker", "x": 390, "y": 700, "w": 300, "h": 300, "fit": "contain", "rotation": 0}},
      "z_index": 20
    }},
    {{
      "type": "date_tag",
...
15. 坐标与尺寸必须结合 layout_policy 动态决定：标题过长时换行或缩小；正文变长时先扩大正文区，再缩小主贴纸并减少装饰
16. 日期、心情、天气横向或分行排列时必须互不遮挡；天气不可用时完全省略 weather_tag
17. 正文不得截断，props.h 必须足以容纳当前字号、行距和换行；页面底部保留 layout_policy.bottom_whitespace_min 指定的留白
18. 主贴纸不得覆盖正文主要区域；decoration/tape/frame 不得覆盖标题、标签、正文或主体中心区域
19. 页面素材总数宁少勿多，整页元素建议不超过 12 个
